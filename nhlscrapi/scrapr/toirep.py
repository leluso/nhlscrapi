...
class TOIRepBase(ReportLoader):
    ...
    def parse_shifts(self):
        ...
        for pl in pl_heads:
            ...
            num, *comma_name_list = pl_text.split()
            num_name = pl_text.replace(',', '').split(' ')
            sh_sum['player_num'] = int(num) if comma_name_list and num.isdigit() else -1
            name = ' '.join(comma_name_list)
            last_name, first_name = name.split(',')
            sh_sum['player_name'] = {'first': first_name.strip(), 'last': last_name.strip()}
